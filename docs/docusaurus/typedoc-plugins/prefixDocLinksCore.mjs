 ...
 */
function splitAngleWrappedDestination(core) {
    let index = 1;
 ...
 * @returns {{ destination: string; remainder: string }}
 */
function splitRawDestination(core) {
    let depth = 0;
    let index = 0;
 ...
 */
function nextCodeSpanLength(codeSpanLength, tickRun) {
 ...
 */
function rewriteInlineLinkAtCloseBracket(line, closeBracketIndex) {
    ...
    return {
        nextIndex: end + 1,
        text: `](${prefixInlineLinkPayload(payload)})`,
    };
}
...
function prefixInlineMarkdownLinksInLine(line) {
    ...
    while (i < line.length) {
        ...
        if (tickRun > 0) {
            codeSpanLength = nextCodeSpanLength(codeSpanLength, tickRun);
            ...
            const rewrite = rewriteInlineLinkAtCloseBracket(line, i);
