...
description: "🤖🤖 Use this prompt to bootstrap a new ESLint plugin repository by migrating the source plugin into my modern template structure. IMPORTANT: treat source ESLint plugin template only as a structural and quality guide, never as rule content to port or convert unless the source plugin already has an equivalent rule."
...
The current repository may have been scaffolded from my `source ESLint plugin template` template, but that does **not** mean you should port, rename, reinterpret, or adapt the Template-specific-specific rules into the new plugin.
...
- do not convert `source ESLint plugin template` rules into rules for the new plugin
...
2. The root of this repository has been scaffolded using my template (based on my `source ESLint plugin template` repo). All root `package.json` dependencies & devDependencies are already installed.
...
D. The entire project matches the layout, doc standards, and coding standards of my `source ESLint plugin template` template **without importing unrelated Template-specific-specific rule content**.
...
1. **Inventory Template-specific-specific rule code, docs, and tests and remove them surgically during migration.**
...
3. **Update all configs, scripts, and docs** to remove any references to Template-specific, template-specific runtime helper, or their rules.
...
**If you are ever unsure, default to preserving infrastructure and only removing clearly obsolete Template-specific-specific rule artifacts once replacement coverage exists.**
